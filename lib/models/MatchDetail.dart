...
// Helper function to convert Map to List if needed
  ...
  Map<String, dynamic> toJson() => {
    ...
    'lineups': {
      'home': lineups.isNotEmpty ? lineups[0].toJson() : null,
      'away': lineups.length > 1 ? lineups[1].toJson() : null,
    },
  };
}

class Lineup {
  final String teamName; // Sửa từ Team thành String
  final Coach coach;
  ...
  factory Lineup.fromJson(Map<String, dynamic> json) {
    print('Lineup JSON: $json'); // Log JSON for debugging
    return Lineup(
      teamName: json['teamName'] ?? '',
      coach: Coach.fromJson(json['coach'] ?? {'name': '', 'photo': ''}),
...
class Coach {
  ...
  Coach({required this.name, required this.photo});
